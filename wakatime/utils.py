    ...
    if entity is not None and entity.strip() != "":
        for pattern in include:
            try:
                ...
                log.warning(
                    u("Regex error ({msg}) for include pattern: {pattern}").format(
        ...
        for pattern in exclude:
            try:
                ...
                log.warning(
                    u("Regex error ({msg}) for exclude pattern: {pattern}").format(
                        msg=u(ex), pattern=u(pattern),
                    )
                )
    return False
...
def get_user_agent(plugin=None):
    ...
    user_agent = u("wakatime/{ver} ({platform}) Python{py_ver}").format(
        ver=u(__version__), platform=u(platform.platform()), py_ver=python_version,
    )
    if plugin:
        user_agent = u("{user_agent} {plugin}").format(
            user_agent=user_agent, plugin=u(plugin),
        )
    else:
        user_agent = u("{user_agent} Unknown/0").format(user_agent=user_agent,)
    return user_agent
