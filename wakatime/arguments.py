...
class FileAction(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        if isinstance(values, (str, bytes)) and values.startswith('"'):
...
def parse_arguments():
    ...
    parser.add_argument(
        "--file", dest="file", action=FileAction, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--key",
        dest="key",
    ...
    parser.add_argument(
        ...
        dest="plugin",
        action=StoreWithoutQuotes,
        help="Optional text editor plugin name and version " + "for User-Agent header.",
    )
    parser.add_argument(
        "--time",
        dest="timestamp",
    ...
    parser.add_argument(
        ...
        action=StoreWithoutQuotes,
        metavar="INT",
        help="Optional line number. This is the current " + "line being edited.",
    )
    parser.add_argument(
        "--cursorpos",
        dest="cursorpos",
    ...
    parser.add_argument(
        "--disableoffline",
        dest="offline_deprecated",
        action="store_true",
        help=argparse.SUPPRESS,
    )
    parser.add_argument(
        "--hide-file-names",
        dest="hide_file_names",
        action="store_true",
    ...
    parser.add_argument(
        "--ignore", dest="ignore", action="append", help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--extra-heartbeats",
        dest="extra_heartbeats",
        action="store_true",
    ...
    parser.add_argument(
        "--logfile", dest="logfile", action=StoreWithoutQuotes, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--api-url",
        dest="api_url",
    ...
    parser.add_argument(
        "--apiurl", dest="apiurl", action=StoreWithoutQuotes, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--timeout",
        dest="timeout",
    ...
    parser.add_argument(
        ...
        metavar="KEY",
        action=StoreWithoutQuotes,
        help="Prints value for the given config key, then " + "exits.",
    )
    parser.add_argument(
        "--config-write",
        dest="config_write",
    ...
    if not args.key:
        ...
        if default_key:
            ...
            try:
                parser.error(
                    "Missing api key. Find your api key from wakatime.com/settings/api-key."
                )
            except SystemExit:
                raise SystemExit(AUTH_ERROR)

    is_valid = not not re.match(
    ...
    if not is_valid:
        try:
            parser.error(
                "Invalid api key. Find your api key from wakatime.com/settings/api-key."
            )
        except SystemExit:
            raise SystemExit(AUTH_ERROR)
    ...
    if not args.entity:
        if args.file:
            ...
            not args.sync_offline_activity or args.sync_offline_activity == "none"
        ) and not args.today and not args.today_goal:
    ...
    try:
        ...
        parser.error(
            'argument --sync-offline-activity must be "none" or an integer number'
        )
    ...
    if configs.has_option("settings", "exclude"):
        try:
            ...
            pass
    if not args.include_only_with_project_file and configs.has_option(
        "settings", "include_only_with_project_file"
    ):
        args.include_only_with_project_file = (
            configs.get("settings", "include_only_with_project_file") == "true"
        )
    if not args.include:
        args.include = []
    if configs.has_option("settings", "include"):
        try:
            ...
            pass
    if not args.exclude_unknown_project and configs.has_option(
        "settings", "exclude_unknown_project"
    ):
        args.exclude_unknown_project = configs.getboolean(
            "settings", "exclude_unknown_project"
        )
    _boolean_or_list(
        "hide_file_names",
        args,
        configs,
        alternative_names=["hide_filenames", "hidefilenames"],
    )
    _boolean_or_list(
        "hide_project_names",
        args,
        configs,
        alternative_names=["hide_projectnames", "hideprojectnames"],
    )
    _boolean_or_list(
        "hide_branch_names",
        args,
        configs,
        alternative_names=["hide_branchnames", "hidebranchnames"],
        default=None,
    )
    ...
    if args.proxy:
        ...
        if not is_valid:
            parser.error(
                "Invalid proxy. Must be in format "
                + "https://user:pass@host:port or "
                + "socks5://user:pass@host:port or "
                + "domain\\user:pass."
            )
